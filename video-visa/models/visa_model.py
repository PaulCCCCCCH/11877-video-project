...
class ViSAModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        parser.set_defaults(norm='batch', netG='AutoEncoder')
        return parser
    ...
    def forward(self):
        if self.isTrain or self.reconst_only:
            self.reconst= self.netG(self.img, 0)
            return self.reconst

        if self.latent_code_only:
            self.latent_code= self.netG(self.img, 1)
            return self.latent_code
        
        if self.decode_only:
            self.reconst = self.netG(self.img, 2)
            return self.reconst

        if self.pixel_feat_only:
            self.f01, self.f02, self.f03, self.f04 = self.netG(self.img, 3)
